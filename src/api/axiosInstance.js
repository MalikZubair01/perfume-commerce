...
// ---- Bulletproof Base URL Resolver ----
const resolveBaseUrl = () => {
  // 1. Check Vite style (import.meta.env)
  if (typeof import.meta !== "undefined" && import.meta.env?.VITE_API_BASE_URL) {
    return import.meta.env.VITE_API_BASE_URL;
  }
  // 2. Check Create React App style (process.env)
  if (typeof process !== "undefined" && process.env?.REACT_APP_API_BASE_URL) {
    return process.env.REACT_APP_API_BASE_URL;
  }
  // 3. Fallback production URL so it never fails silently
  return "https://perfume-backend-production-a7ac.up.railway.app/api";
};

const API_BASE_URL = resolveBaseUrl();
console.log("Resolved API Base URL is:", API_BASE_URL);
